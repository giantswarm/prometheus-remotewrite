...
statefulset_name = "prometheus-mimir-to-grafana-cloud"
...
def pods(kube_cluster: Cluster) -> List[pykube.Pod]:
    ...
    pods = pods.filter(namespace=namespace_name, selector={
                       'app.kubernetes.io/name': 'prometheus', 'app.kubernetes.io/instance': 'mimir-to-grafana-cloud'})

    return pods

# when we start the tests on circleci, we have to wait for pods to be available, hence
# this additional delay and retries
# -- Checking that all pods from prometheus-mimir-to-grafana-cloud's statefulset are available (i.e in "Ready" state)
@pytest.mark.smoke
@pytest.mark.upgrade
